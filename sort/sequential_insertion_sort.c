/**
 * @file 
 * @brief A program to sort an array of elements using the sequential insertion sort algorithm
 */

#include <assert.h> // for assert function (for tests)

/**
 * @brief Sorts an array of integers using the sequential insertion sort algo
 * @param arr the pointer to the array of elements
 * @param length the number of elements in the array
 * @returns void
 */
void sequentialInsertionSort(int arr[], int length) {
    int temp, element;
    for(int i = 1; i < length; i++) {
        element = i;
        
        for(int j = i - 1; j >= 0; j--, element--) {
            if(arr[element] >= arr[j]) break;
...
void test(void) {
    ...
    sequentialInsertionSort(test1, 5); // sorts test array 1
    sequentialInsertionSort(test2, 5); // sorts test array 2
    sequentialInsertionSort(test3, 6); // sorts test array 3
    sequentialInsertionSort(test4, 1); // sorts test array 4
